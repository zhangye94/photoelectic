    ...
    <el-tabs v-model="activeName">
          ...
          <div class="card-form" v-loading="loading">
            <el-form ref="form" :model="form" label-width="180px" size="mini">
              <el-form-item label="掩模主图形优化步长">
                <div>:<span class="paramContent">{{form.stepMaskMain}}</span></div>
              </el-form-item>
              <el-form-item label="掩模辅助图形优化步长">
                <div>:<span class="paramContent">{{form.stepMaskSraf}}</span></div>
              </el-form-item>
              <el-form-item label="光源图形优化步长">
                <div>:<span class="paramContent">{{form.stepSource}}</span></div>
              </el-form-item>
              <el-form-item label="掩模罚函数权重因子">
                <div>:<span class="paramContent">{{form.omegaMaskQua}}</span></div>
              </el-form-item>
              <el-form-item label="光源罚函数权重因子">
                <div>:<span class="paramContent">{{form.omegaSourceQua}}</span></div>
              </el-form-item>
              <el-form-item label="最大循环次数">
                <div>:<span class="paramContent">{{form.maxloopSmo}}</span></div>
              </el-form-item>
      ...
      <el-tab-pane label="Convergence" name="Convergence">
        <div class="top-card" v-loading="loading">
          <div class="card-body">
            <div class="top-card-text">Iteration number:<span>{{form.maxloopSmo || 0}}</span></div>
        ...
        <div style="overflow: hidden;">
          <div class="img-card" v-loading="loading">
            ...
            <div class="notRunning-contain" v-show="!gdResultSmo.sourcePatternPng">
              <div class="notRunning">
                <img src="../../assets/images/result.png" alt="result" class="result-icon">
                <span>点击 Start 后可查看运行结果</span>
              </div>
            </div>
            <div class="img-card-body" v-show="gdResultSmo.sourcePatternPng">
              <img :src="ip + '/smo/' + gdResultSmo.sourcePatternPng" class="output-img" @click="handleImgClick('sourcePatternPng')">
              <i class="el-icon-zoom-in"></i>
              <el-dialog :visible.sync="dialogVisible.sourcePatternPng">
                <img width="100%" :src="ip + '/smo/' + gdResultSmo.sourcePatternPng" alt="服务器未找到该图片">
              </el-dialog>
            </div>
          </div>
          <div class="img-card" v-loading="loading">
            <div class="card-header">Mask Pattern</div>
            <div class="notRunning-contain" v-show="!gdResultSmo.maskPatternPng">
              <div class="notRunning">
                <img src="../../assets/images/result.png" alt="result" class="result-icon">
                <span>点击 Start 后可查看运行结果</span>
              </div>
            </div>
            <div class="img-card-body" v-show="gdResultSmo.maskPatternPng">
              <img :src="ip + '/smo/' + gdResultSmo.maskPatternPng" class="output-img" @click="handleImgClick('maskPatternPng')">
              <i class="el-icon-zoom-in"></i>
              <el-dialog :visible.sync="dialogVisible.maskPatternPng">
                <img width="100%" :src="ip + '/smo/' + gdResultSmo.maskPatternPng" alt="服务器未找到该图片">
              </el-dialog>
            </div>
          </div>
          <div class="img-card" v-loading="loading">
            <div class="card-header">Print Image</div>
            <div class="notRunning-contain" v-show="!gdResultSmo.printImagePng">
              <div class="notRunning">
                <img src="../../assets/images/result.png" alt="result" class="result-icon">
                <span>点击 Start 后可查看运行结果</span>
              </div>
            </div>
            <div class="img-card-body" v-show="gdResultSmo.printImagePng">
              <img :src="ip + '/smo/' + gdResultSmo.printImagePng" class="output-img" @click="handleImgClick('printImagePng')">
              <i class="el-icon-zoom-in"></i>
              <el-dialog :visible.sync="dialogVisible.printImagePng">
                <img width="100%" :src="ip + '/smo/' + gdResultSmo.printImagePng" alt="服务器未找到该图片">
              </el-dialog>
            </div>
          </div>
          <div class="img-card" v-loading="loading">
            <div class="card-header">Convergence Curve</div>
            <div class="notRunning-contain" v-show="!gdResultSmo.errorConvergencePng">
              <div class="notRunning">
                <img src="../../assets/images/result.png" alt="result" class="result-icon">
                <span>点击 Start 后可查看运行结果</span>
              </div>
            </div>
            <div class="img-card-body" v-show="gdResultSmo.errorConvergencePng">
              <img :src="ip + '/smo/' + gdResultSmo.errorConvergencePng" class="output-img" @click="handleImgClick('errorConvergencePng')">
              <i class="el-icon-zoom-in"></i>
              <el-dialog :visible.sync="dialogVisible.errorConvergencePng">
                <img width="100%" :src="ip + '/smo/' + gdResultSmo.errorConvergencePng" alt="服务器未找到该图片">
              </el-dialog>
            </div>
          </div>
          <div class="img-card" v-loading="loading">
            <div class="card-header">加权误差收敛曲线</div>
            <div class="notRunning-contain" v-show="!gdResultSmo.errorConvergenceWeightPng">
              <div class="notRunning">
                <img src="../../assets/images/result.png" alt="result" class="result-icon">
                <span>点击 Start 后可查看运行结果</span>
              </div>
            </div>
            <div class="img-card-body" v-show="gdResultSmo.errorConvergenceWeightPng">
              <img :src="ip + '/smo/' + gdResultSmo.errorConvergenceWeightPng" class="output-img" @click="handleImgClick('errorConvergenceWeightPng')">
              <i class="el-icon-zoom-in"></i>
              <el-dialog :visible.sync="dialogVisible.errorConvergenceWeightPng">
                <img width="100%" :src="ip + '/smo/' + gdResultSmo.errorConvergenceWeightPng" alt="服务器未找到该图片">
              </el-dialog>
            </div>
          </div>
        </div>
        <div class="top-card" style="margin-top: 15px;">
          ...
          <div class="card-body" v-loading="loading">
            <ul class="download-doc-list">
              <li><i class="el-icon-document"></i><a :href="ip + '/smo/' + gdResultSmo.errorMat" target="_blank">{{gdResultSmo.errorMat}}</a></li>
              <li><i class="el-icon-document"></i><a :href="ip + '/smo/' + gdResultSmo.errorWeightMat" target="_blank">{{gdResultSmo.errorWeightMat}}</a></li>
              <li><i class="el-icon-document"></i><a :href="ip + '/smo/' + gdResultSmo.maskPatternMat" target="_blank">{{gdResultSmo.maskPatternMat}}</a></li>
              <li><i class="el-icon-document"></i><a :href="ip + '/smo/' + gdResultSmo.printImageMat" target="_blank">{{gdResultSmo.printImageMat}}</a></li>
              <li><i class="el-icon-document"></i><a :href="ip + '/smo/' + gdResultSmo.sourcePatternMat" target="_blank">{{gdResultSmo.sourcePatternMat}}</a></li>
            </ul>
          </div>
        </div>
      </el-tab-pane>
    ...
    methods: {
      getSmoParam() {
        this.$http.post('api/get_record_detail.do',{
          ...
          this.form = res.data.data.gdParameterSmo;
          this.inputMask = res.data.data.gdParameterSmo.inputMask;
          this.gdResultSmo = res.data.data.gdResultSmo;
