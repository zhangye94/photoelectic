      ...
      <el-tab-pane label="Convergence" name="Convergence">
        <div class="top-card">
          <div class="card-body">
            ...
            <div class="top-card-text">Error Weight:<span>{{intermediateResult.errorWeight || 0}}</span></div>
          </div>
        </div>
        <div style="overflow: hidden;">
          <div class="img-card">
            <div class="card-header">Source Pattern</div>
            <div class="notRunning-contain" v-show="!intermediateResult.opcSourcePatternPng">
              <div class="notRunning">
                <img src="../../assets/images/result.png" alt="result" class="result-icon">
                <span>未运行或暂无中间结果</span>
              </div>
            </div>
            <div class="img-card-body" v-show="intermediateResult.opcSourcePatternPng">
              <img :src="'http://' + ip + '/inter/' + intermediateResult.opcSourcePatternPng" class="output-img" @click="handleImgClick('opcSourcePatternPng')">
              <i class="el-icon-zoom-in"></i>
              <el-dialog :visible.sync="dialogVisible.opcSourcePatternPng">
                <img width="100%" :src="'http://' + ip + '/inter/' + intermediateResult.opcSourcePatternPng" alt="服务器未找到该图片">
              </el-dialog>
            </div>
          </div>
          <div class="img-card">
            <div class="card-header">Mask Pattern</div>
            <div class="notRunning-contain" v-show="!intermediateResult.opcMaskPatternPng">
              <div class="notRunning">
                <img src="../../assets/images/result.png" alt="result" class="result-icon">
                <span>未运行或暂无中间结果</span>
              </div>
            </div>
            <div class="img-card-body" v-show="intermediateResult.opcMaskPatternPng">
              <img :src="'http://' + ip + '/inter/' + intermediateResult.opcMaskPatternPng" class="output-img" @click="handleImgClick('opcMaskPatternPng')">
              <i class="el-icon-zoom-in"></i>
              <el-dialog :visible.sync="dialogVisible.opcMaskPatternPng">
                <img width="100%" :src="'http://' + ip + '/inter/' + intermediateResult.opcMaskPatternPng" alt="服务器未找到该图片">
              </el-dialog>
            </div>
          </div>
          <div class="img-card">
            <div class="card-header">Print Image</div>
            <div class="notRunning-contain" v-show="!intermediateResult.opcPrintImagePng">
              <div class="notRunning">
                <img src="../../assets/images/result.png" alt="result" class="result-icon">
                <span>未运行或暂无中间结果</span>
              </div>
            </div>
            <div class="img-card-body" v-show="intermediateResult.opcPrintImagePng">
              <img :src="'http://' + ip + '/inter/' + intermediateResult.opcPrintImagePng" class="output-img" @click="handleImgClick('opcPrintImagePng')">
              <i class="el-icon-zoom-in"></i>
              <el-dialog :visible.sync="dialogVisible.opcPrintImagePng">
                <img width="100%" :src="'http://' + ip + '/inter/' + intermediateResult.opcPrintImagePng" alt="服务器未找到该图片">
              </el-dialog>
            </div>
          </div>
          <div class="img-card">
            <div class="card-header">Convergence Curve</div>
            <div class="notRunning-contain" v-show="!intermediateResult.opcErrorConvergencePng">
              <div class="notRunning">
                <img src="../../assets/images/result.png" alt="result" class="result-icon">
                <span>未运行或暂无中间结果</span>
              </div>
            </div>
            <div class="img-card-body" v-show="intermediateResult.opcErrorConvergencePng">
              <img :src="'http://' + ip + '/inter/' + intermediateResult.opcErrorConvergencePng" class="output-img" @click="handleImgClick('opcErrorConvergencePng')">
              <i class="el-icon-zoom-in"></i>
              <el-dialog :visible.sync="dialogVisible.opcErrorConvergencePng">
                <img width="100%" :src="'http://' + ip + '/inter/' + intermediateResult.opcErrorConvergencePng" alt="服务器未找到该图片">
              </el-dialog>
            </div>
          </div>
          <div class="img-card">
            <div class="card-header">加权误差收敛曲线</div>
            <div class="notRunning-contain" v-show="!intermediateResult.opcErrorConvergenceWeightPng">
              <div class="notRunning">
                <img src="../../assets/images/result.png" alt="result" class="result-icon">
                <span>未运行或暂无中间结果</span>
              </div>
            </div>
            <div class="img-card-body" v-show="intermediateResult.opcErrorConvergenceWeightPng">
              <img :src="'http://' + ip + '/inter/' + intermediateResult.opcErrorConvergenceWeightPng" class="output-img" @click="handleImgClick('opcErrorConvergenceWeightPng')">
              <i class="el-icon-zoom-in"></i>
              <el-dialog :visible.sync="dialogVisible.opcErrorConvergenceWeightPng">
                <img width="100%" :src="'http://' + ip + '/inter/' + intermediateResult.opcErrorConvergenceWeightPng" alt="服务器未找到该图片">
              </el-dialog>
            </div>
          </div>
        </div>
...
<script>
  export default {
    ...
    data () {
      return {
        dialogVisible: {
          opcErrorConvergencePng: false,
          opcPrintImagePng: false,
          opcMaskPattrenPng: false,
          opcErrorConvergenceWeightPng: false,
          opcSourcePatternPng: false
        },
        ...
        this.$http.post('api/get_running_state.do',{
          ...
          if(this.loopFlag){
            if(res.data.status === 9){
              ...
              if(res.data.data.opcErrorConvergencePng || res.data.data.opcPrintImagePng || res.data.data.opcMaskPattrenPng || res.data.data.opcSourcePatternPng){
...
//                this.$router.push({ path: '/simPancel/opcView', query: { parametersId: res.data.data.parametersId, resultId: res.data.data.id, type: 'Convergence'}});
            }
          }
        }, (err) => {
          this.$message({
      ...
      checkSim() {
        if(this.inputMask != ''){
          ...
          this.$http.post('api/opc_parameters_simulated.do',{
            ...
            na: this.form.na,
            ratio: this.form.ra,
            stepMain: this.form.sm,
            stepSraf: this.form.ssr,
            omegaQua: this.form.omq,
            maxloopOpc: this.form.ma,
            ...
            if(res.data.status === 3){
              this.$confirm('该组参数已经仿真过，是否需要重新仿真？', '提示', {
                ...
                this.$router.push({ path: '/simPancel/opcView', query: { parametersId: res.data.data.parametersId, resultId: res.data.data.resultId, type: 'Convergence'}});
              });
            }else{
              this.$message({
                message: res.data.msg,
      ...
      startSim() {
        ...
        this.$http.post('api/opc_simulate.do',{
          maskDimension: this.form.md,
          pixelSize: this.form.ps,
          reflect: this.form.re,
          absorb: this.form.ab,
    ...
    watch: {
      '$route' (to, from) {
        if(from.path === '/simPancel/opc'){
          this.loopFlag = false;
        }
        if(to.path === '/simPancel/opc'){
          this.loopFlag = true;
          this.getIfRunning();
        }
      }
